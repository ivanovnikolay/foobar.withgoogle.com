...
def solution(m):
    ''' Markov chain absorb '''
    S = [sum(r) for r in m]
    Q = Matrix([Fraction(e, S[i]) for j, e in enumerate(r) if S[j]] for i, r in enumerate(m) if S[i])
    R = Matrix([Fraction(e, S[i]) for j, e in enumerate(r) if not S[j]] for i, r in enumerate(m) if S[i])
    if Q and R:
        I = Matrix.I(n=len(Q))
        F = ~(I - Q)
        s0 = (F * R)[0]
